...
Release: `main` is the release branch. The version in `manifest.json`, `package.json` and the new key in `versions.json`, with the `CHANGELOG.md` entry, go in one commit on `main`; `git push origin main`; `npm run build` at that commit; `git tag <version>` with no `v` prefix; `git push origin <version>`; `gh release create <version> --title "ICOR for Life - AI Chat <version>" main.js manifest.json styles.css`. The `publish` branch the older flow pushed from (`git push origin publish:main`) is dead and is not the release source; it stopped tracking `main` before 0.14.0. `main.js` is gitignored and exists only as a release asset, so it is built at the tagged commit and nowhere else: the Obsidian directory rebuilds from the tagged source and compares byte for byte, and an asset built from any other tree will not match. The directory reads the release assets.
