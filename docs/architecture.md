...
`src/team/snapshotParse.ts` and `src/team/snapshot.ts` are the life snapshot, split the same way `memoryParse.ts` and `memory.ts` are: the pure half parses `.icor-for-life/scripts/snapshot.json` (schema 1, written by the vault's `life-snapshot.py`), renders the twenty-line brief and states the three reader lines for stale, missing and degraded; the Obsidian half fetches the bytes through `this.app.vault.adapter`, `exists` first, because a hidden folder is invisible to the `Vault` API and no vault event ever fires for it (GL-1008). `ChatView` reads it on open for the header line and again before the first send, and `withSnapshot` puts the block in front of that one message and no other. The plugin never runs the script and never writes the file. The brief is rendered twice in two languages on purpose (the script renders it for the session-start hook, this renders it for the chat); `brief()` in `life-snapshot.py` is the shape of record, and the two label strings that differ from it today are the 2026-09-15 naming ruling, which the script follows in its next pass.

**Honest limit: a runtime that cannot say who ran.** The Codex provider's normaliser (`src/provider/codex/normalize.ts`) emits no `subagent-start` and no `subagent-end`, and hands every event the main thread, so a Codex archive records zero subagents. Zero is not a measurement here, it is a missing one, and reading it as a measurement made Team Insights show Larry alone on every Codex session. So `src/team/insights.ts` keeps one string table, `NO_SUBAGENT_DETAIL`: a session whose `provider` is in it contributes no participant at all, is out of the agent ranking and out of its denominator, and says `Specialist detail is not available for Codex yet` on its own row, with the count of excluded sessions printed beside the ranking. `src/team/load.ts` reads the runtime through `manifestProvider`, so archives already written get the note without being rewritten. Two facts the table does not fix and does not hide. A Codex session's tool calls all land under `TOOLS USED · MAIN THREAD`, because that is where the normaliser put them. And `counts.subagents` in a manifest is no session's own count on any runtime: it is written from `plugin.subagents.all()`, a register that spans the plugin's lifetime and is never emptied, so it reads as high as 47 on a Codex folder whose `agents` list is empty. Nothing in the insights path reads that field, which is why the ranking was wrong rather than merely inflated, and fixing it is its own piece of work.

The way out is normalisation, not presentation. Codex 0.154.0 publishes `subAgentActivity` and `collabAgentToolCall`, and `Thread.parentThreadId` gives the parent a child can be routed by, which is enough to produce the two events and a real `stream` id per child; `src/provider/codex/session.ts` currently discards the thread id it is handed. That work is version-gated, needs a recorded fixture, and needs the stream-and-turn-ownership contract ruled on first (a child must never complete the parent's turn). A runtime leaves `NO_SUBAGENT_DETAIL` the day its normaliser emits the two events, and not before.
...
## Where the own-key engine's keys live

The own-key engine (`src/engine/`, the direct Anthropic and OpenRouter path) is the only part of the plugin that holds a credential. Since 0.13.0 the key is never in `data.json` and never in the per-device local-storage record (`src/engine/credentials.ts` keeps the engine, provider, model, budget, the chosen backend and the env-file path, and its writer cannot emit a key field). The key lives in one of two backends behind one door, `src/engine/secrets.ts`: Obsidian's keychain (`app.secretStorage`, 1.11.4 and newer, ids `icor-for-life-chat-<provider>-api-key`) or an env file in the vault (`src/engine/envFile.ts`, a pure parser and writer that rewrites one line and leaves every other byte as it was). The member picks the backend; `readProviderKey` reads that backend and never the other, so a wrong setting shows as a missing key rather than working by accident. `secretStorageOf` narrows the app structurally instead of through the `App` type, because the scanner's `no-unsupported-api` rule resolves `App.secretStorage` to its 1.11.4 `@since` and the floor is 1.8.7 on purpose: the plugin runs there, with the env file as its only backend. `main.ts` runs `migratePlaintextKeys` before `loadSettings` so a leftover plaintext key is moved into the store before the settings reader can copy it back. The settings section (`src/settings/EngineSection.ts`) shows where each key exists and moves one on a button; the paste field is write-only. Tests: `test/engine-envfile.test.mjs`, `test/engine-secrets.test.mjs`, `test/engine-credentials.test.mjs`.
...
Release: `main` is the release branch. The version in `manifest.json`, `package.json` and the new key in `versions.json`, with the `CHANGELOG.md` entry, go in one pull request to `main`, squash merged. Then `git tag <version>` on that merge commit, with no `v` prefix, and `git push origin <version>`. Nothing else is done by hand: the tag push runs `.github/workflows/release.yml`, which calls the shared release in `myICOR/.github`. Its guard refuses a tag that is not the `manifest.json` version or not on `main`. Its build job checks out the tagged commit, installs exactly the lockfile (`npm ci`) on the Node in `.node-version`, runs `npm run build`, and hands `main.js` with `manifest.json` and `styles.css` from that same tree to the release job, which publishes the three. Its provenance job signs them (`gh attestation verify main.js --repo myICOR/icor-for-life-chat --signer-workflow myICOR/.github/.github/workflows/obsidian-release.yml`). Its verify job rebuilds the tag in a separate job and is red unless the published `main.js` equals the rebuild. `main.js` is gitignored and exists only as a release asset, so it is built at the tagged commit and nowhere else: the Obsidian directory rebuilds from the tagged source and compares byte for byte, and an asset built from any other tree will not match. The directory reads the release assets. To rehearse the pipeline without cutting a release, run the Release workflow by hand with an existing tag (`gh workflow run release.yml -R myICOR/icor-for-life-chat -f tag=<current version>`): a manual run is always a dry run, and it tags, publishes and signs nothing. The `publish` branch the older flow pushed from (`git push origin publish:main`) is dead and is not the release source; it stopped tracking `main` before 0.14.0.
