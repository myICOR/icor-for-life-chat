...
export default defineConfig([
    ...
    rules: {
      ...
      'obsidianmd/ui/sentence-case': ['warn', {
        /* 'Anthropic' / 'OpenRouter' added 2026-09-06 (mobile hardening,
           ...
           way the runtimes above them are. 'Remote Control' is Claude Code's
           own feature name (0.12.0). */
        brands: ['Claude Code', 'ICOR', 'Obsidian', 'AI Sessions', 'Bypass', 'Remote Control', 'Anthropic', 'OpenRouter'],
  ...
  },
  {
    /* THE THIRD FILE-SCOPED EXEMPTION, with its reason (2026-09-06, mobile
       ...
       `remoteControl/adapters.ts` joined the list at the 0.13.0 merge
       (Flint finding 2): its `spawnDetached` reaches `node:child_process`
       and is on `ChatView.ts`'s static import graph.
    ...
    files: ['src/main.ts', 'src/provider/cli.ts', 'src/provider/registry.ts', 'src/remoteControl/adapters.ts'],
    rules: {
