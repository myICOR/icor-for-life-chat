...
/** `./cli` the same way (0.13.0 merge, Flint finding 2): `ChatView.ts`
 * resolves a Linux terminal emulator for Remote Control with
 * `resolveExecutable`, and a static import of `./cli` there would evaluate
 * its `node:fs` / `node:path` on mobile the instant the plugin loads. The
 * one caller runs on the desktop only; this is the door it goes through. */
let cliReal: typeof import('./cli') | null = null;
export function cliModule(): typeof import('./cli') {
  cliReal ??= require('./cli') as typeof import('./cli');
  return cliReal;
}
