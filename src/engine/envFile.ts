...
/** The sentence for a path the plugin will not read or write, or null when
 * the path is inside the vault (Vex A-1, 0.13.0). The two sentences are the
 * Planner's `normalizeEnvFilePath` ones, word for word, so the suite refuses
 * the same paths in the same words: an absolute path (a leading `/`, `\\`,
 * or a drive letter), a home shortcut (`~`), or any `..` segment. Obsidian's
 * adapter does not resolve `..`, so a setting of `../x` would read and write
 * the parent of the vault. */
export function vaultPathRefusal(raw: string): string | null {
  const s = raw.trim().replace(/\\/g, '/');
  if (s.startsWith('/') || s.startsWith('~') || /^[A-Za-z]:/.test(s)) {
    return 'The path is relative to the vault root, not an absolute path.';
  }
  if (s.split('/').includes('..')) return 'The path must stay inside the vault (no "..").';
  return null;
}

export function isOutsideVault(raw: string): boolean {
  return vaultPathRefusal(raw) !== null;
}

/** A vault-relative path as the adapter wants it: forward slashes, no
 * leading `./`, trimmed. Empty stays empty so a caller can fall back to the
 * default rather than reading the vault root by accident, and a path that
 * points outside the vault (`isOutsideVault`) comes back empty for the same
 * reason: the default is the one place the plugin knows is inside. */
export function cleanVaultPath(raw: string): string {
  if (isOutsideVault(raw)) return '';
